...
public class ImageUtility
{
   ...
   * @return the color version of the image
   */
  public static int[][] n2argb(double[] Is, int w, int h)
  {
  	int[][] Isi = new int[h][w];
  	
  	for(int x=0; x<w; x++){
  		for(int y=0; y<h; y++){
  			Isi[y][x] = (int)Is[y*w+x];
  		}
  	}
  ...
  /**
	 * Copy an ARGB image.
	 * @param img0 the image to copy
	 * @return the copy of the image
	 */
	public static int[][] copy(int[][] img0)
	{
		int h = img0.length;
		int w = img0[0].length;
		int[][] img1 = new int[h][w];
	 ...
	 * @param rgb_old the old color
	 * @param rgb_new the new color
	 */
	public static void replace(BufferedImage image, int rgb_old, int rgb_new)
	{
		int rgb;
		
		for(int x=0; x<image.getWidth(); x++){
			for(int y=0; y<image.getHeight(); y++){
				rgb = image.getRGB(x, y) & 0x00ffffff;
				
				if(rgb == rgb_old){
					image.setRGB(x, y, rgb_new);
				}
			}
		}
	}
	...
	/**
	 * Duplicate the foreground of an image.
	 * @param image the image
	 * @param foreground_color the color of the foreground
	 * @param background_color the desired background color
	 * @param n the number of times to duplicate the foreground
	 * @param xy_offset the offset between each duplicate
	 * @param rgb_offset the offset between colors in duplicates
	 * @return the new image
	 */
	public static BufferedImage duplicateForeground(BufferedImage image, int foreground_color, int background_color, int n, int xy_offset, int rgb_offset)
	{
		BufferedImage new_image = new BufferedImage(image.getWidth(), image.getHeight(), BufferedImage.TYPE_4BYTE_ABGR);
		int xy0 = -(n*xy_offset)/2;
		int rgb, r, g, b, xi, yi;
		
		//Set background
		for(int x=0; x<new_image.getWidth(); x++){			
			for(int y=0; y<new_image.getHeight(); y++){
				new_image.setRGB(x, y, background_color);
			}
		}
		...
		for(int i=n+1; i>0; i--){				
			for(int x=0; x<new_image.getWidth(); x++){
				...
				if(xi >= 0 && xi < new_image.getWidth()-1){
					for(int y=0; y<new_image.getHeight(); y++){
						yi = xy0 + y;
						
						if(yi >= 0 && yi < new_image.getHeight()-1){
							rgb = image.getRGB(x, y);
							
							if(rgb == foreground_color){
								if(i > 1){
									r = (rgb >> 16) & 0x000000ff;
									g = (rgb >> 8) & 0x000000ff;
									b = rgb & 0x000000ff;
  ...
  public static int difference(double[] diff_img, int[] img0, int[] img1, int threshold, boolean SHOW_DIFFERENCE)
  {
	 ...
	 * @return the cropped image
	 */
	public static double[][] crop(double[][] img, int minx, int miny, int maxx, int maxy)
	{
		int h = img.length;
		int w = img[0].length;
		double[][] img_cropped;
		...
		w_cropped = maxx-minx+1;
		h_cropped = maxy-miny+1;
		img_cropped = new double[h_cropped][w_cropped];
	...
	/**
	 * Crop a black and white image so as to remove black padding.
	 * @param img a black and white image
	 * @return the cropped image
	 */
	public static double[][] crop(double[][] img)
	{
		int h = img.length;
		int w = img[0].length;
		int minx = -1;
  ...
  public static double[][] resize(double[][] img, int w_new, int h_new)
  {
	...
	/**
   * Resize a grayscale image.
   * @param img1D the image
   * @param w_new the desired image w_new
   * @param h_new the desired image h_new
   * @return the resized image
   */
  public static double[] resize(double[] img1D, int w, int h, int w_new, int h_new)
  {
  	double[][] img2D = MatrixUtility.to2D(h, w, img1D);
    
    return MatrixUtility.to1D(resize(img2D, w_new, h_new));
  }
   ...
   * @return the resized image
   */
  public static double[][] resizeBicubic(double[][] img, double scale)
  {
  	int h = img.length;
  	int w = img[0].length;
    int w_new = (int)Math.round(scale * w);
    ...
    return resizeBicubic(img, w_new, h_new);
  }
   ...
   * @return the resized image
   */
  public static double[] resizeBicubic(double[] img1D, int w, int h, int w_new, int h_new)
  {
  	double[][] img2D = MatrixUtility.to2D(h, w, img1D);
    
    return MatrixUtility.to1D(resizeBicubic(img2D, w_new, h_new));
  }
   ...
   * @return the resized image
   */
  public static double[] resizeBicubic(double[] img1D, int w, int h, double scale)
  {
  	double[][] img2D = MatrixUtility.to2D(h, w, img1D);
    int w_new = (int)Math.round(scale * w);
    int h_new = (int)Math.round(scale * h);
   ...
   * @param option indicates whether or not to maintain the image dimensions or the image content
   ...
   */
  public static double[] translate(double[] I, int w, int h, int tx, int ty, double default_value)
  {
  	double[] I_new = MatrixUtility.vector(w*h, default_value);
  	int x_old, y_old;
   ...
   */
  public static int[][] rotate(int[][] Irgb, double theta, double xc, double yc)
  {
  	int h = Irgb.length;
  	int w = Irgb[0].length;
  	int[][] Irgb_r = new int[h][w];
  	int x0, y0;
  ...
  public static double[][] smooth(double[][] Ig, int iterations)
  {
	 ...
	 * @param Ibw a black and white image
	 * @param w the width of the image
	 * @param h the height of the image
	 */
	public static void thin(double[] Ibw, int w, int h)
	{		
			...
			for(int x=1; x<w-1; x++){
				for(int y=1; y<h-1; y++){
					...
					if(Ibw[c] > 0){
						...
						if((Ibw[tl]>0 && Ibw[t]>0 && Ibw[tr]>0 && Ibw[br]==0 && Ibw[b]==0 && Ibw[bl]==0) ||	//Top
							 (Ibw[t]>0 && Ibw[tr]>0 && Ibw[r]>0 && Ibw[b]==0 && Ibw[bl]==0 && Ibw[l]==0) ||		//Top right
							 (Ibw[tr]>0 && Ibw[r]>0 && Ibw[br]>0 && Ibw[bl]==0 && Ibw[l]==0 && Ibw[tl]==0) ||	//Right
							 (Ibw[r]>0 && Ibw[br]>0 && Ibw[b]>0 && Ibw[l]==0 && Ibw[tl]==0 && Ibw[t]==0) ||		//Bottom right
							 (Ibw[br]>0 && Ibw[b]>0 && Ibw[bl]>0 && Ibw[tl]==0 && Ibw[t]==0 && Ibw[tr]==0) ||	//Bottom
							 (Ibw[b]>0 && Ibw[bl]>0 && Ibw[l]>0 && Ibw[t]==0 && Ibw[tr]==0 && Ibw[r]==0) ||		//Bottom left
							 (Ibw[bl]>0 && Ibw[l]>0 && Ibw[tl]>0 && Ibw[tr]==0 && Ibw[r]==0 && Ibw[br]==0) ||	//Left
							 (Ibw[l]>0 && Ibw[tl]>0 && Ibw[t]>0) && Ibw[r]==0 && Ibw[br]==0 && Ibw[b]==0){		//Top Left
							Ibw[c] = 0;
							THINNING = true;
						}
					}
	...
	/**
	 * Thin the given binary image using Hilditch skeletonization.
	 * @param Ibw a black and white image
	 * @param w the width of the image
	 * @param h the height of the image
	 */
	public static void hilditch(double[] Ibw, int w, int h)
	{		
		int c, t, b, l, r, tl, tr, bl, br;
		double[] N = new double[8];
		double[] A = new double[Ibw.length];
		double[] B = new double[Ibw.length];
		boolean[] F = new boolean[Ibw.length];
		boolean THINNING = true;
		...
		while(THINNING){
			...
			for(int x=1; x<w-1; x++){
				for(int y=1; y<h-1; y++){
					...
					for(int i=0; i<8; i++){
						if(N[i] > 0 && N[(i+1)%8] == 0) A[c]++;
						if(N[i] > 0) B[c]++;
					}
				}
			}
			
			//Flag pixels to remove
			for(int x=1; x<w-1; x++){
				for(int y=1; y<h-1; y++){
					c = y*w+x;
					...
					if(Ibw[c] > 0){
						...
						if((B[c] >= 2 && B[c] <= 6) &&
							 (A[c] == 1) &&
							 ((Ibw[t] == 0 || Ibw[r] == 0 || Ibw[b] == 0) || A[r] != 1) &&
							 ((Ibw[t] == 0 || Ibw[r] == 0 || Ibw[l] == 0) || A[t] != 1)){
							F[c] = true;
							THINNING = true;
						}
					}
	...
	/**
	 * Thicken the given binary image.
	 * @param Ibw a black and white image
	 * @param w the width of the image
	 * @param h the height of the image
	 * @return the thickened binary image
	 */
	public static double[] thicken(double[] Ibw, int w, int h)
	{			
		double[] Ibw_new = MatrixUtility.copy(Ibw);
  ...
  /**
   * Apply a distance transform to the given black and white image.
   * @param Ibw a black and white image
   * @return the distance transform of the given black and white image
   */
   ...
   */
  public static void drawPoint(int[] img, int w, int h, Pixel p, int color, int r)
  {
  	int minx = p.x - r;
  	int maxx = p.x + r;
  	int miny = p.y - r;
   ...
   *  @param x1 the x-coordinate of the ending point of the segment
   *  @param y1 the y-coordinate of the ending point of the segment
  ...
  {
  	drawLine(img, w, h, x0, y0, x1, y1, color, 0);
  }
   ...
   *  @param y1 the y-coordinate of the ending point of the segment
   *  @param color the color of the segment in ARGB
   *  @param r the thickness of the lines
   */
  public static void drawLine(int[] img, int w, int h, int x0, int y0, int x1, int y1, int color, int r)
  {
    double dx = x1 - x0;
    double dy = y1 - y0;
    double m, b;   
      ...
      while(x0 < x1){
        ...
        if(y0 >= 0 && y0 < h){
        	if(r == 0){
        		...
        		drawPoint(img, w, h, new Pixel(x0, y0), color, r);
        	}
        }
      }
    }else{
   ...
   * @param w the width of the image
   * @param h the height of the image
   * @param lines the line segments to draw
   * @param color the color of the segment in ARGB
   * @param r the thickenss of the lines
   */
	public static void drawLines(int[] image, int w, int h, Vector<Pixel[]> lines, int color, int r)
	{
		for(Pixel[] line : lines){
			ImageUtility.drawLine(image, w, h, line[0].x, line[0].y, line[1].x, line[1].y, color, r);
		}
	}
   ...
   */
	...
	{
		drawLines(image, w, h, lines, color, 0);
	}
  ...
  /**
	 * Create an image with an alpha mask based on the specified minimum color thresholds.
	 * @param image the image
	 * @param minr the red threshold for masked pixels
	 * @param ming the green threshold for masked pixels
	 * @param minb the blue threshold for masked pixels
	 * @return the image with a set alpha mask
	 */
	public static BufferedImage getAlphaImage(BufferedImage image, int minr, int ming, int minb)
	{
		BufferedImage masked_image = new BufferedImage(image.getWidth(), image.getHeight(), BufferedImage.TYPE_4BYTE_ABGR);
		int rgb, r, g, b;
		
		//Copy image to an image with an alpha value
		for(int x=0; x<image.getWidth(); x++){
			for(int y=0; y<image.getHeight(); y++){
				masked_image.setRGB(x, y, image.getRGB(x, y));
			}
		}
		
		//Mask out colors according to thresholds
		for(int x=0; x<masked_image.getWidth(); x++){
			for(int y=0; y<masked_image.getHeight(); y++){
				rgb = masked_image.getRGB(x, y);
				r = (rgb & 0x00ff0000) >> 16;
				g = (rgb & 0x0000ff00) >> 8;
				b = rgb & 0x000000ff;
  ...
  private static double[][] flog(int w, int h, double sigma)
  {
  	...
  	//To make consistent with matlab version start pixel indexing at 1.
  	for(int x = 0; x < w; x++){
  		for(int y = 0; y < h; y++){
  			xtmp = x - xr;
  			ytmp = y - yr;
  ...
  /**
   * Invert the intensity of a black and white image.
   * @param img a black and white image
   * @return the negative of the image
   */
  public static double[] getNegative(double[] img)
  {
  	double[] img_negative = new double[img.length];
  	...
  	//Invert values
  	for(int i=0; i<img.length; i++){
  		img_negative[i] = (1-(img[i]-min)/max)*max + min;
  	}
   ...
   * @param mask the binary image whose white pixels we wish to group
   * @param r the distance with which to merge two groups
   * @param USE_EIGHT_NEIGHBORHOOD true if a eight neighborhood should be used (r is ignored if false)
   * @return the list of points within each group
   */
  public static Vector<Vector<Pixel>> getGroups(double[][] mask, int r, boolean USE_EIGHT_NEIGHBORHOOD)
  {
  		...
  		for(int y=0; y<h; y++){
  			if(mask[y][x] > 0 && !marked[y][x]){
  		      ...
  		      if(USE_EIGHT_NEIGHBORHOOD){
  		      	...
  		      	if(p.x >= 1 && mask[p.y][p.x-1] > 0 && !marked[p.y][p.x-1]){
		      			stk.push(new Pixel(p.x-1, p.y));
  		      	}
  		      	
  		      	if(p.x < w-1 && mask[p.y][p.x+1] > 0 && !marked[p.y][p.x+1]){
		      			stk.push(new Pixel(p.x+1, p.y));
  		      	}
  		      	
  		      	if(p.y >= 1 && mask[p.y-1][p.x] > 0 && !marked[p.y-1][p.x]){
		      			stk.push(new Pixel(p.x, p.y-1));
  		      	}
  		      	
  		      	if(p.y < h-1 && mask[p.y+1][p.x] > 0 && !marked[p.y+1][p.x]){
		      			stk.push(new Pixel(p.x, p.y+1));
  		      	}
  		      }
  		  	}
   ...
   * @return the list of points within each group
   */
  public static Vector<Vector<Pixel>> getGroups(double[][] mask)
  {
  	return getGroups(mask, 1, true);
  }
  
  /**
   * Get the mean value of each group.
   * @param groups list of points within each group
   * @return the mean of each group
   */
  public static Vector<Pixel> getGroupMeans(Vector<Vector<Pixel>> groups)
  {
  	Vector<Pixel> means = new Vector<Pixel>();
  	int sumx, sumy;
  	...
  	for(int i=0; i<groups.size(); i++){
  		...
  		for(int j=0; j<groups.get(i).size(); j++){
  			sumx += groups.get(i).get(j).x;
  			sumy += groups.get(i).get(j).y;
  		}
  		
  		means.add(new Pixel(sumx/groups.get(i).size(), sumy/groups.get(i).size()));
  	}
  ...
  /**
   * Get the largest group from a list of pixel groups.
   * @param groups groups of pixels
   * @return the index of the largest group
   */
  public static int getLargestGroup(Vector<Vector<Pixel>> groups)
  {
  	int maxs = 0;
  	int maxi = 0;
  	
  	for(int i=0; i<groups.size(); i++){
  		if(groups.get(i).size() > maxs){
  			maxs = groups.get(i).size(); 
  			maxi = i;
  		}
  	}
   ...
   */
  public static double[] getGroups(Vector<Vector<Pixel>> groups, int w, int h)
  {
   ...
   * @param h the height of the image
   * @param r the distance with which to merge two groups
   * @param USE_EIGHT_NEIGHBORHOOD true if a eight neighborhood should be used
   * @return the group labels of each pixel in the black and white image
   */
  public static double[] getGroups(double[] Id, int w, int h, int r, boolean USE_EIGHT_NEIGHBORHOOD)
  {
  	Vector<Vector<Pixel>> groups = getGroups(MatrixUtility.to2D(h, w, Id), r, USE_EIGHT_NEIGHBORHOOD);
  	
  	return getGroups(groups, w, h);
  }
  ...
  {
  	return getGroups(Id, w, h, 1, true);
  }
  ...
  public static Vector<Vector<Pixel>> getRegionBorders(double[] img, int w, int h, boolean CLIP_BOUNDARIES)
  {
	 ...
	 */
	public static double[][] getPointMatrix(double[] image, int w, int h)
	{
		...
		//Convert to a matrix
		X = new double[pixels.size()][2];
		
		for(int i=0; i<pixels.size(); i++){
			X[i][0] = pixels.get(i).x;
			X[i][1] = pixels.get(i).y;
		}
  ...
  public static double[][] getGeodesicActiveContour(double[][] Ie)
  {
	...
	/**
	 * Get the end points in a binary image.
	 * @param image a black and white image
	 * @param w the width of the image
	 * @param h the height of the image
	 */
	public static Vector<Pixel> getEndPoints(double[] image, int w, int h)
	{
		Vector<Pixel> points = new Vector<Pixel>();
		int minu, maxu, minv, maxv;
		int neighbors;
			...
			for(int y=0; y<h; y++){
				if(image[y*w+x] > 0.5){
					...
					for(int u=minu; u<=maxu; u++){
						for(int v=minv; v<=maxv; v++){
							if(u != x || v != y){
								if(image[v*w+u] > 0.5) neighbors++;
							}
						}
					}
					
					if(neighbors == 1) points.add(new Pixel(x, y));
				}
			}
		}
	...
	/**
	 * Get the outline of the given black and white image.
	 * @param ig a black and white image
	 * @param w the width of the image
	 * @param h the height of the image
	 * @return the outline of the image
	 */
	public static double[] getOutline(double[] ig, int w, int h)
	{
		double[] ig_outline = MatrixUtility.vector(w*h, 0);
		int minx, maxx, miny, maxy;
		boolean BORDER_PIXEL;
			...
			for(int y=0; y<h; y++){
				if(ig[y*w+x] < 0.5){
					...
					for(int u=minx; u<=maxx; u++){
						...
						if(BORDER_PIXEL) break;
					}
					
					if(BORDER_PIXEL) ig_outline[y*w+x] = 1;
				}
			}
		}
  ...
  /**
   * Remove border groups by merging them into the largest group.
   * @param groups groups of pixels
   * @param w the width of the image
   * @param h the height of the image
   * @return a new group of pixels
   */
  public static Vector<Vector<Pixel>> mergeBorderGroups(Vector<Vector<Pixel>> groups, int w, int h)
  {
  	Vector<Vector<Pixel>> groups_new = new Vector<Vector<Pixel>>();
  	int maxi = ImageUtility.getLargestGroup(groups);
  	Vector<Pixel> largest_group = groups.get(maxi);
  	boolean BORDER_GROUP;
  	Pixel p;
  	...
  	for(int i=0; i<groups.size(); i++){
  		if(i != maxi){
	  		...
	  		if(BORDER_GROUP){
	  			largest_group.addAll(groups.get(i));
	  		}else{
	  			groups_new.add(groups.get(i));
	  		}
  		}
  	}
  	
  	groups_new.add(largest_group);
  	
  	return groups_new;
  }
  
  /**
   * Render text in a specified font.
   * @param text the text to render
   * @param fontname the name of the font to use
   * @param width the widht of the rendered to image
   * @param height the height of the rendered to image
   * @return the image containing the rendered text
   */
  public static BufferedImage renderText(String text, String fontname, int width, int height)
  {
  	BufferedImage image = new BufferedImage(width, height, BufferedImage.TYPE_INT_ARGB);
		Graphics2D g2 = image.createGraphics();
		int fontsize = (int)Math.round(0.75 * image.getHeight());
		...
		g2.drawString(text, 50, image.getHeight()/2-g2.getFontMetrics().getMaxDescent()/2+g2.getFontMetrics().getMaxAscent()/2+15);
