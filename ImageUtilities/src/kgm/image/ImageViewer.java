...
/**
 * A modified JFrame that allows for easily popping up windows containing images based on 
 * arrays, matrices, or image data.
 * @author Kenton McHenry
 */
public class ImageViewer extends JFrame implements Runnable, MouseListener, MouseWheelListener, KeyListener
{
  ...
  private boolean BUFFER_EVENTS = false;
  private int clicked_x = 0;
  private int clicked_y = 0;
  private int clicked_button = 0;
  private Queue<Triple<Integer,Integer,Integer>> click_buffer = new LinkedList<Triple<Integer,Integer,Integer>>();
  private Queue<Integer> scroll_buffer = new LinkedList<Integer>();
   ...
   * @param width the width of the frame (note: height is set automatically!)
	 */
	public ImageViewer(String title, int width)
	{
	  this(new int[100*100], 100, 100, width, title);
	  CLEAR_ON_ADD = true;
	}
  ...
  /**
	 * Enable/disable event buffering.
	 * @param value true if events should be stored for later use
	 */
	public void enableEventBuffer(boolean value)
	{
		BUFFER_EVENTS = value;
	}
  ...
  /**
	...
	/**
	 * Get a buffered click.
	 * @return the clicked point
	 */
	public Pixel getBufferedClick()
	{
		Triple<Integer,Integer,Integer> triple = click_buffer.poll();
		
		return new Pixel(triple.first, triple.second);
	}

	/**
	 * Get a buffered click triple (x, y, button).
	 * @return the (x, y, button) triple
	 */
	public Triple<Integer,Integer,Integer> getBufferedClickTriple()
	{  	
		return click_buffer.poll();
	}
	
	/**
	 * Get a buffered scroll wheel event.
	 * @return the direction of the wheel (-1=up, 1=down, null=nothing)
	 */
	public Integer getBufferedWheelEvent()
	{
		return scroll_buffer.poll();
	}
  ...
  {     
    ...
    if(BUFFER_EVENTS){
    	click_buffer.add(new Triple<Integer,Integer,Integer>(clicked_x, clicked_y, clicked_button));
    }
  ...
  {  	
  	int scroll_direction = e.getWheelRotation() < 0 ? -1 : 1;
    
  	if(BUFFER_EVENTS){
    	scroll_buffer.add(scroll_direction);
    }
  ...
  public void keyTyped(KeyEvent e) {}
	...
	/**
   * The starting point for the automatically started thread that continuously updates the frame.
   */
  public void run()
  {  
    while(true){
  ...
  public static void main(String args[])
  {
  	if(true){
	  	...
	  	ImageViewer viewer = new ImageViewer(image, image[0].length, image.length);
  	}
  }
}